...
describe('basic', () => {
  ...
  beforeEach(() => {
    ...
    mocked(Sample).mockImplementation(() => <RealSample />)
  ...
  it('render', () => {
    ...
    expect(app.getByAltText('Vite logo')).toBeInTheDocument()
    expect(app.getByText('Vite + React')).toBeInTheDocument()
    expect(app.getByRole('button', { name: 'count is 0' })).toBeInTheDocument()
    expect(document.querySelector('.card > p')?.textContent).toEqual('Edit src/App.tsx and save to test HMR')
    expect(app.getByText('Click on the Vite and React logos to learn more')).toBeInTheDocument()
    const sampleBoldText = app.getByText('/___service_backend___')
    expect(sampleBoldText).toBeInTheDocument()
    const sampleContainerClassList = (sampleBoldText.parentNode?.parentNode as HTMLElement)?.classList
...
describe('substituting react component using jest.mock', () => {
  it('should render app with plain DOM', () => {
    mocked(Sample).mockImplementation(() => <div data-testid="sample">Some Sample</div>)
  ...
  it('should render app with with sample component', () => {
    const RealSample = jest.requireActual('./components/Sample').default
    mocked(Sample).mockImplementation(() => (
